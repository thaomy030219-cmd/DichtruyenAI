...
export const autoAnalyzeStory = async (
    ...
    for (let i = 0; i < chunks.length; i += CONCURRENCY) {
        ...
        const batchPromises = batch.map((chunk) => {
            ...
            const flashModels = ['gemini-3.6-flash', 'gemini-3-flash-preview', 'gemini-3.5-flash'].filter(id => enabledModels?.includes(id) ?? true);
            if (flashModels.length === 0) flashModels.push('gemini-3.6-flash', 'gemini-3-flash-preview', 'gemini-3.5-flash');
