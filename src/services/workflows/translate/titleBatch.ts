...
`;
    ...
    let candidates = ['gemini-3.6-flash', 'gemini-3-flash-preview', 'gemini-3.5-flash-lite', 'gemini-3.1-flash-lite'].filter(id => enabledModels?.includes(id) ?? true);
    if (candidates.length === 0) candidates = ['gemini-3.6-flash', 'gemini-3-flash-preview', 'gemini-3.5-flash-lite', 'gemini-3.1-flash-lite']; // Fallback if none enabled
