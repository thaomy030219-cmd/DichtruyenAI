...
export const useContextAnalysisHandlers = (core: any, ui: any, automation: any) => {
    ...
    const handleNameAnalysis = async (config: { mode: 'only_char' | 'full' | 'deep_context'; scope: 'smart' | 'range' | 'full'; rangeStart: number; rangeEnd: number; updatedStoryInfo: StoryInfo; useSearch: boolean; additionalRules?: string; sampling?: {start: number, middle: number, end: number} }) => {
        ...
        try {
            ...
            // Chế độ Nhanh: ưu tiên hết quota 3.6 Flash > 3.5 Flash > 3.0 Flash preview
            const QUICK_CHAIN = ['gemini-3.6-flash', 'gemini-3.5-flash', 'gemini-3-flash-preview'];
            // Chế độ Sâu, từ batch 4 trở đi: 3.6 Flash > 3.5 Flash > 3.0 Flash preview
            const DEEP_LATE_CHAIN = ['gemini-3.6-flash', 'gemini-3.5-flash', 'gemini-3-flash-preview'];
                ...
                const batchPromises = batch.map(async (chunk, idx) => {
                    ...
                    if (isDeep) {
                        if (batchNum <= 3) {
                            ...
                            models = idx % 2 === 0
                                ? filterEnabled(['gemini-3.1-pro-preview', 'gemini-3.6-flash', 'gemini-3.5-flash', 'gemini-3-flash-preview'])
                                : filterEnabled(['gemini-3.6-flash', 'gemini-3.5-flash', 'gemini-3-flash-preview', 'gemini-3.1-pro-preview']);
                        } else {
                            // Batch 4 trở đi: toàn bộ 3.6 Flash, hết quota mới rớt xuống 3.5 rồi 3.0 preview
                            models = filterEnabled(DEEP_LATE_CHAIN);
                        }
                    } else {
                        // Chế độ Nhanh: 3.6 Flash > 3.5 Flash > 3.0 Flash preview
                    ...
                    try {
                        ...
                        const rescueModels = filterEnabled(['gemini-3.6-flash', 'gemini-3.5-flash', 'gemini-3-flash-preview']);
                        try {
                            if (config.mode === 'deep_context') {
