            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[80vh]">
                <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-primary-100 text-primary-600 rounded-xl"><History className="w-5 h-5" /></div>
                    ...
                    {CHANGELOG_DATA.map((entry, idx) => (
                        <div key={idx} className="mb-6 last:mb-0">
                            <div className="flex items-center gap-2 mb-3">
                                <span className="bg-primary-600 text-white px-2 py-0.5 rounded-lg text-[10px] font-bold">v{entry.version}</span>
                                ...
                                {entry.changes.map((c, i) => {
                                    ...
                                    return (
                                        <li key={i} className="text-sm text-slate-600 flex gap-3">
                                            <div className="p-1.5 bg-slate-50 rounded-lg shrink-0 h-fit"><Icon className="w-3.5 h-3.5 text-primary-500" /></div>
