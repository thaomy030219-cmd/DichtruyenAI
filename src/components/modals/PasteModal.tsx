        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200 p-6">
                ...
                <label className="flex items-center gap-2 cursor-pointer mb-2">
                    <input type="checkbox" checked={isTranslated} onChange={e => setIsTranslated(e.target.checked)} className="rounded text-primary-600 focus:ring-primary-500" />
                ...
                <div className="flex justify-end gap-3 mt-4">
                    ...
                    <button onClick={() => { if(content.trim()) { onConfirm(title, content, isTranslated); onClose(); setTitle(''); setContent(''); setIsTranslated(false); } }} className="px-6 py-2 bg-primary-600 text-white rounded font-bold">Xác Nhận</button>
                </div>
            </div>
        </div>
    );
