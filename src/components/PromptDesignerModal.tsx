            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-300 relative flex flex-col max-h-[90vh]">
                {isOptimizing && (
                    <div className="absolute inset-0 bg-white/95 z-20 flex flex-col items-center justify-center p-8 text-center">
                        <div className="relative mb-6">
                            ...
                            <Wrench className="absolute inset-0 m-auto w-8 h-8 text-primary-500 animate-pulse" />
                        </div>
                ...
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-primary-50/30">
                    <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2">
                        <Wrench className="w-5 h-5 text-primary-500"/> Prompt Architect (Tối Ưu Hóa)
                    </h3>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
                    ...
                    <div className="space-y-3 pt-4 border-t border-slate-100">
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] bg-primary-50 text-primary-600 px-2 py-0.5 rounded-full">New v10.1</span>
                            </div>
                            <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleRefineAdditionalRules(); }} className="text-[10px] flex items-center gap-1 font-bold text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/30 px-2 py-1 rounded-md hover:bg-primary-100 transition-colors">
                    ...
                    <div className="space-y-3 pt-4 border-t border-slate-100">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <label className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${useContext ? 'bg-primary-50 border-primary-200' : 'bg-white border-slate-200'}`}>
                                <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${useContext ? 'bg-primary-500 border-primary-500' : 'bg-white border-slate-300'}`}>
                ...
                <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3 flex-shrink-0">
                    ...
                    <button 
                        ...
                        className="px-6 py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl font-bold text-sm shadow-lg shadow-primary-200 transition-all flex items-center gap-2"
